...
class MLCommonsSSDLargeModel(Model):
    ...
    def postprocess(self, outputs: Sequence[np.ndarray], extra_params: Dict[str, Any], confidence_threshold=0.3) -> Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray]]:
        ...
        for boxes, labels, scores in zip(det_boxes, det_labels, det_scores):
            boxes, labels, scores = self.pick_best(detections=(boxes, labels, scores), confidence_threshold=confidence_threshold)
